...
class ProfileRepository(
    ...
    suspend fun getUserProfile(): ProfileResult<UserProfile> = runProfileRequest {
        service.getUserProfile()
    }

    /** Streams profile updates in realtime. */
    fun observeUserProfile(): Flow<UserProfile> = service.observeUserProfile()

    /** Updates profile fields and settings. */
    suspend fun updateProfile(profile: UserProfile): ProfileResult<Unit> = runProfileRequest {
    ...
    suspend fun changePassword(currentPassword: String, newPassword: String): ProfileResult<Unit> = runProfileRequest {
    ...
    suspend fun deleteAccount(): ProfileResult<Unit> = runProfileRequest {
        service.deleteAccount()
    }

    /** Uploads a profile image and returns its download URL. */
    suspend fun uploadProfileImage(imageUri: Uri): ProfileResult<String> = runProfileRequest {
